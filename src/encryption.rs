...
fn encrypt_block(data: &[u8], key: &[u8]) -> [u8; 8] {
    ...
    for _ in 0..3 {
        for j in 0..8 {
            ...
            hashed_block = encryption_step(hashed_block, key_part, false);
        }
    }
    ...
    hashed_block = encryption_step(hashed_block, key_part, true);
...
fn encryption_step(block_to_encrypt: Vec<u8>, key_part: &[u8], is_last_step: bool) -> Vec<u8> {
    // apply encryption function to right half of block
    let hashed_prev_right_half = encryption_step_fn(&block_to_encrypt[4..8], key_part);
...
fn encryption_step_fn(data: &[u8], key_part: &[u8]) -> [u8; 4] {
